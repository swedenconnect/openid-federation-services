...
/**
 * {@link ReadyStateComponent} for starting trust anchor.
 *
 * @author Felix Hellman
 */
@Component
public class TrustAnchorInitializer extends ReadyStateComponent {
  ...
  @EventListener
  TrustAnchorInitializedEvent handle(final EntitiesLoadedEvent event) {
    this.source.getTrustAnchorProperties().stream()
        .map(TrustAnchorRegistrationEvent::new)
        .forEach(this.publisher::publishEvent);
    this.markReady();
    return new TrustAnchorInitializedEvent();
  }

  @Override
  protected String name() {
    return "trust-anchor-init";
  }
}
