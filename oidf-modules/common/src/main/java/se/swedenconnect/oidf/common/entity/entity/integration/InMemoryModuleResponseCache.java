...
/**
 * In-memory implementation of {@link ModuleResponseCache}.
 *
 * @author Felix Hellman
 */
public class InMemoryModuleResponseCache implements ModuleResponseCache {

  private final Map<Long, Map<String, CachedResponse>> cache = new ConcurrentHashMap<>();
  ...
  public Optional<CachedResponse> get(final long snapshot, final String requestUri) {
    return Optional.ofNullable(this.cache.get(snapshot))
        .flatMap(version -> Optional.ofNullable(version.get(requestUri)));
  }
