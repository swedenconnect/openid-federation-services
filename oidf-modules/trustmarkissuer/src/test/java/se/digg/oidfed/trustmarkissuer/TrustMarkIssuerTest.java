...
import se.digg.oidfed.trustmarkissuer.dvo.EntityId;
...
class TrustMarkIssuerTest {
  ...
  private ConfigurationResolverInMemory configurationResolver;

  @BeforeEach
  public void init() throws Exception {
    configurationResolver = TestDataSetup.createConfigurationResolver();
  }
  ...
  void trustMarkListing() throws NotFoundException, InvalidRequestException {
    ...
    final TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties sub1 =
        TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties.builder()
            .sub(EntityId.create("http://sub1.se").toString())
    ...
    final TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties sub2 =
        TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties.builder()
            .sub(EntityId.create("http://sub2.se").toString())
            .expires(Instant.now().plus(10, ChronoUnit.DAYS))
            .granted(Instant.now())
            .build();
    ...
    final TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties sub3 =
        TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties.builder()
            .sub(EntityId.create("http://sub.outdated.se").toString())
            .expires(Instant.now().plus(1, ChronoUnit.DAYS))
            .granted(Instant.now())
            .build();

    configurationResolver.addTrustMarkIssuerProperties(TrustMarkIssuerProperties.builder()
        .trustmarkid(TrustMarkId.create("http://tm1.digg.se"))
        .subjects(List.of(sub1, sub2, sub3))
        .build());

    final TrustMarkIssuer trustMarkIssuer = new TrustMarkIssuer(configurationResolver) {
      @Override
    ...
    assertThrows(InvalidRequestException.class,
        () -> trustMarkIssuer.trustMarkListing(new TrustMarkListingRequest(null, null, null)));
    assertThrows(NotFoundException.class,
        () -> trustMarkIssuer.trustMarkListing(new TrustMarkListingRequest(null, "http://www.i.trust.u.se", null)));

    final List<String> search1 =
        trustMarkIssuer.trustMarkListing(new TrustMarkListingRequest(null, "http://tm1.digg.se", null));
    assertEquals(2, search1.size());

    final List<String> search2 = trustMarkIssuer.trustMarkListing(
        new TrustMarkListingRequest(null, "http://tm1.digg.se", "http://sub.notfound"));
    assertEquals(0, search2.size());

    final List<String> search3 =
        trustMarkIssuer.trustMarkListing(new TrustMarkListingRequest(null, "http://tm1.digg.se", "http://sub1.se"));
    assertEquals(1, search3.size());
    assertEquals("http://sub1.se", search3.get(0));

    final List<String> search4 = trustMarkIssuer.trustMarkListing(
        new TrustMarkListingRequest(null, "http://tm1.digg.se", "http://sub.outdated.se"));
  ...
  public void testTrustMarkValidity()
    ...
    final TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties expired =
        TrustMarkIssuerProperties.TrustMarkIssuerSubjectProperties.builder()
            .sub(EntityId.create("http://expired.se").toString())
    ...
    assertTrue(trustMarkIssuer.trustMarkStatus(new TrustMarkStatusRequest("http://tm1.digg.se","http://sub1.se",null)));
    assertFalse(trustMarkIssuer.trustMarkStatus(new TrustMarkStatusRequest("http://tm1.digg.se","http://expired.se",null)));
  }

  @Test
  void testExpCalculationExpectTTLForSubject() throws NotFoundException, InvalidRequestException {
  ...
  @Test
  void testExpCalculationExpectDuration() throws NotFoundException, InvalidRequestException {
